use crate::models::{Category, Tier, Section};
...
impl Database {
    ...
    fn run_migrations(&self) -> AnyhowResult<()> {
        ...
        let mut migrations_dir = "backend/core/migrations";
    ...
    pub fn query_memory(
        ...
        let mut stmt = self.conn.prepare(&sql).context("Failed to prepare memory query")?;
