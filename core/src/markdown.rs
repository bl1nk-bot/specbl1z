...
/// เขียนบล็อก tree ลงฐานข้อมูล (ภายใน Transaction)
fn insert_blocks_tx(
    ...
    for (idx, block) in blocks.iter().enumerate() {
        ...
        if !block.children.is_empty() {
            insert_blocks_tx(tx, document_id, &block.children, Some(&block.id))?;
        }
    }
    Ok(())
}

/// นำเข้า Markdown ทั้งหมดเข้าสู่ฐานข้อมูล แบบ Transactional (Atomic & Fast)
pub fn import_markdown(
    ...
    // metadata
    let metadata_str = serde_json::to_string(&frontmatter.unwrap_or(serde_yaml::Value::Null))?;
    let doc_id = new_id();

    // รันทุกอย่างใน Transaction เดียวกันเพื่อประสิทธิภาพ
    db.transaction(|tx| {
        // สร้างเอกสาร
        tx.execute(
            "INSERT INTO documents (id, title, folder_id, metadata) VALUES (?1, ?2, ?3, ?4)",
            rusqlite::params![doc_id, title, folder_id, metadata_str],
        )?;

        // แปลงและบันทึกบล็อก
        let blocks = parse_markdown_to_blocks(&body);
        insert_blocks_tx(tx, &doc_id, &blocks, None)?;
