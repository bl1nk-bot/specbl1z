...
### First Run (codebase ใหม่)

ทำ full codebase scan ทุกไฟล์ก่อน จากนั้น:

1. รายงานสรุป — จัดลำดับความรุนแรงจากมากไปน้อย (Critical / High / Medium)
2. เลือก **1 ไฟล์ที่มีปัญหามากที่สุด** มาแก้ไข
3. ห้ามแก้เกิน 1 ไฟล์ต่อรัน

รูปแบบรายงาน:
```
[SCAN REPORT]
Critical: src/auth/middleware.ts — redundant null-guards, 8 obvious comments, 2 dead params
High:     src/utils/helpers.ts — excessive abstraction, 3 one-use variables
Medium:   src/components/Form.tsx — generic variable names (data, result, item)

→ Fixing: src/auth/middleware.ts
```

### Normal Run (รันที่ 2 เป็นต้นไป)

ตรวจสอบเฉพาะไฟล์ที่ถูกเปลี่ยนแปลง:
- พบปัญหา → แก้ไขเจาะจง **1 ไฟล์** เท่านั้น
- ไม่พบปัญหา → ไม่ต้องทำอะไร
...
**เป้าหมาย: ลบ ceremony ออก ไม่ใช่ลบ functionality**

สำหรับ pattern catalog ฉบับเต็ม ดู `references/code-patterns.md`

### 10 Patterns หลัก

1. **คอมเมนต์ที่ยืดยาวเกินไป** — อธิบายซ้ำสิ่งที่โค้ดทำอยู่แล้ว
   ```python
   # Bad: # Create a user
   user = User()
   # Good: (ลบคอมเมนต์ออก)
...
2. **การป้องกันเกินจำเป็น** — null-check, try-catch, validation ที่ไม่จำเป็น
   ```ts
   // Bad
...
3. **การประกาศ type ซ้ำซ้อน** — type ที่ compiler อนุมานได้อยู่แล้ว
   ```ts
   const count: number = items.length  // Bad
   const count = items.length           // Good
   ```

4. **Boilerplate ระเบิด** — class/function/ไฟล์แยกสำหรับงานเล็กที่ควรเป็น expression เดียว

5. **Abstraction เกินเหตุ** — Interface ที่มี 1 implementation, Factory ที่สร้างของเดียว, Strategy pattern สำหรับ 2 กรณี

6. **ชื่อตัวแปรยาวและไม่ชัดเจน**
   ```ts
   currentUserAuthenticationStatusBoolean  // Bad
   isAuthenticated                          // Good
   ```

7. **ตัวแปรกลางที่ไม่จำเป็น** — ใช้ครั้งเดียวในบรรทัดถัดไป
   ```ts
   // Bad
...
8. **Error handling แบบซ้ำซาก** — try-catch ที่ copy-paste กันหลายที่โดยไม่ abstract

9. **JSDoc/docstring ที่เป็น filler** — ไม่ได้เพิ่มข้อมูลอะไรเลย
   ```ts
   /** Gets the user. @param id The id. @returns The user. */
   getUser(id: string)  // Bad — ลบออก หรือเขียนเฉพาะ edge case
   ```

10. **โค้ด "เผื่อไว้ก่อน"** — parameter ที่ไม่ได้ใช้, dead code, feature สำหรับอนาคตที่ยังไม่เกิด
